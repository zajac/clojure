...
public class Compiler implements Opcodes {
    public static long staticFnsCount = 0;
...
//static final Symbol SYNTAX_QUOTE = Symbol.intern("clojure.core", "syntax-quote");
    ...
    static final Handle BOOTSTRAP_INVOKE_HANDLE = new Handle(H_INVOKESTATIC, "clojure/lang/VarBootstrap", "bootstrapInvoke", BOOTSTRAP_METHOD_SIGNATURE);
...
//		SYNTAX_QUOTE, null,
    ...
    static final public Var VAR_META_FORM = Var.create().setDynamic();
    ...
    static final public Keyword dynamicLinkingKey = Keyword.intern("dynamic-linking");
...
//            System.out.println("Remaining: " + RT.keys(m));
    ...
    static class DefExpr implements Expr {
        ...
        public DefExpr(String source, int line, int column, Var var, Expr init, Expr meta, boolean initProvided, boolean isDynamic, boolean shadowsCoreMapping, boolean toEmit) {
            this.source = source;
            this.line = line;
            this.column = column;
            this.var = var;
...
//			else
        ...
        public static long totalVars = 0;
        public static long staticFnVars = 0;
...
//					.without(Keyword.intern(null, "static"));
                ...
                if (RT.booleanCast(getCompilerOption(dynamicLinkingKey)) &&
                    ...
                    if (init instanceof FnExpr && ((FnExpr) init).canBeDirect) {
                        ...
                        return new DefExpr(source, line, column, v, init, meta, initProvided, isDynamic, shadowsCoreMapping, false);
                    } else {
                        Expr meta = mm.count() == 0 ? null : analyze(context == C.EVAL ? context : C.EXPRESSION, mm);
                        String source = (String) SOURCE.deref();
                        int line = lineDeref();
                        int column = columnDeref();
                        return new DefExpr(source, line, column, v, init, meta, initProvided, isDynamic, shadowsCoreMapping, true);
                    }
                } else {
                    Expr meta = mm.count() == 0 ? null : analyze(context == C.EVAL ? context : C.EXPRESSION, mm);
                    String source = (String) SOURCE.deref();
    ...
    public static boolean isConstantMeta(Object form) {
        //TODO
        return true;
    }
...
//					System.out.println(fn.name + " use this");
                ...
                fn.hasPrimSigs = interfaces.size() > 0;
                if (fn.canBeDirect) {
                    ...
                    if (RT.booleanCast(getCompilerOption(dynamicLinkingKey)) &&
                            VAR_META_FORM.isBound()) {
                        try {
                            java.lang.reflect.Method varMetaMethod = Class.forName("clojure.lang.VarInit").getMethod("varMeta");
                            methods = RT.conj(methods,
                                    NewInstanceMethod.parse(
                                            fn,
                                            RT.list(Symbol.intern("varMeta"), RT.vector(Symbol.intern("this")), VAR_META_FORM.deref()),
                                            null,
                                            Collections.singletonMap(NewInstanceMethod.msig("varMeta", new Class[0]), varMetaMethod)));
                            interfaces.add("clojure/lang/VarInit");
                        } catch(Throwable e) {
                            throw Util.sneakyThrow(e);
                        }
                    }
...
//			}
            ...
            Files.write(Paths.get("/Users/jetzajac/fnscount.txt"), Collections.singletonList("" + staticFnsCount));
            System.out.println("staticFnsCount = " + staticFnsCount);
        } catch (LispReader.ReaderException e) {
